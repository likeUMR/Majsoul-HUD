...
rapidjson::Value create_response(const Request &req, rapidjson::Document &doc)
{
    ...
    // The combined shanten-down + tegawari search path can recurse into an
    // unstable graph expansion and crash the native server on some hands.
    // Keep tegawari, which preserves the stronger same-shanten shape search,
    // and disable shanten-down in the backend safety mode.
    if (config.enable_shanten_down && config.enable_tegawari) {
