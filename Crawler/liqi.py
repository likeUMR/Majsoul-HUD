...
class LiqiProto:
    def __init__(self):
        ...
        json_path = os.path.join(current_dir, 'proto', 'liqi.json')
        self.jsonProto = json.load(open(json_path, 'r', encoding='utf-8'))
        ...
        if msg_type == MsgType.Notify:
            ...
            if msg_type == MsgType.Req:
                ...
                self.res_type[msg_id] = (method_name, getattr(
                    liqi_pb2, proto_domain['responseType']))
                ...
                if msg_id in self.res_type:
                    ...
                    method_name = "Unknown_Res"
                    dict_obj = {"error": "Request not found for this Response"}
                    
        result = {'id': msg_id, 'type': msg_type.value,
